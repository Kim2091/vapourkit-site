...
On Linux, setup requires Python 3.12 or 3.13 with `venv` support, `ffmpeg` and `ffprobe` on `PATH`, and a working Vulkan loader and GPU driver. Vapourkit does not install these host prerequisites for you.
...
The bundled comparison viewer is available on Windows. On Linux, install the `video-compare` command with your distribution's package manager, then restart Vapourkit. Comparison is not available for segment-only output.
